    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-end sm:items-start p-4 pointer-events-none">
            {/* Backdrop for mobile interaction primarily, but we make it click-through mostly or just an overlay */}
            ...
            <div className="pointer-events-auto w-full max-w-sm rounded-xl border bg-card text-card-foreground shadow-xl sm:mt-20">
              <div className="flex items-center justify-between border-b p-4">
                <h3 className="font-semibold leading-none tracking-tight">Your Cart</h3>
                <button 
                    onClick={toggleCart} 
              ...
              <div className="p-4 max-h-[60vh] overflow-y-auto space-y-4">
                {cart.map((item, index) => ( // Using index as key fallback if ID is not unique enough in this simple case
                  <div key={`${item.id}-${index}`} className="flex items-center justify-between space-x-4">
